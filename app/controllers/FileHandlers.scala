...
object FileHandlers extends Controller {
   ...
   def interpret = VisitAction { implicit req =>
    ...
    result._1 match {
      case IntpResults.Success => Ok( HtmlRepl.resultToHtml(HtmlRepl.repl.valueOfTerm(HtmlRepl.repl.mostRecentVar)) )
    ...
    req.visit.user match {
      ...
      case Some(user) => {
        ...
        root.findItem(titles.split("/").toList) match {
          ...
          case Some(f: File) => {
            val contentRes = SafeCode.runCode { HtmlRepl.repl.interpret(f.content) }
            contentRes._1 match {
              case IntpResults.Success | IntpResults.Incomplete => {
                val testRes = SafeCode.runCode { HtmlRepl.repl.interpret(f.tests) }
                testRes._1 match {
                  case IntpResults.Success | IntpResults.Incomplete => {
